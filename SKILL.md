...
Build and run Instatic sites: one Bun container holds the visual canvas, content engine, media, auth, forms, plugins, and publisher; published pages ship as plain HTML with hashed CSS bundles. This skill manages a self-hosted Instatic deployment and everything you can do with it. Knowledge was compiled against v0.0.20; later versions can drift, re-verify before trusting behavior.
...
1. **Verify against the pinned checkout before claiming behavior.** Upstream prose drifts from source: their docs say 38 capabilities, the source array has 40.
2. **Session cookie is HttpOnly, Path=/admin.** Scripted clients keep a cookie jar and call under `/admin/api/cms/*`.
...
4. **Step-up gates the big moves:** publish, branch merge/update, user delete, plugin install/lifecycle, token creation. A 401 with step-up context means `POST /auth/step-up` first, then retry inside the window (default 15 min).
...
9. **Verify with DOM measurements and fetched HTML, never eyeballs or image files.** Operator confirmation from his own browser gates interactive flows; an HTTP 200 never marks work done.
10. **Pin image tags; migrations run at boot and a failed migration exits the process.** Keep the previous tag locally until the operator confirms the upgrade.
...
3. Make one logical change to one table or page.
4. Verify per `instatic-cms/verification.md`, then check in.
